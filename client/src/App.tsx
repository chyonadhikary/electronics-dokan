...
function HomePage({ products, site, brands, navigate, addToCart }: { products: Product[]; site: SiteConfig; brands: Brand[]; navigate: (path: string) => void; addToCart: (id: string, qty?: number) => void }) {
  ...
  return <div className="reference-home">
    <section className="reference-hero focused-banner"><div className="focused-banner-slides">{banners.map((banner, index) => <img key={`${banner}-${index}`} className={`focused-banner-slide ${index === activeBanner ? "is-active" : ""}`} src={banner} alt={`Electronics Dokan banner ${index + 1}`} onError={(event) => { event.currentTarget.style.display = "none"; }} />)}</div>{banners.length > 1 && <div className="focused-banner-dots">{banners.map((banner, index) => <button key={`${banner}-dot`} className={index === activeBanner ? "is-active" : ""} onClick={() => setActiveBanner(index)} aria-label={`Show banner ${index + 1}`} />)}</div>}</section>
    ...
    <section className="reference-category-band"><div className="container"><SectionHeading eyebrow="Browse the catalogue" title="Popular categories" action="View all categories" onAction={() => navigate('/categories')} /><div className="reference-category-grid">{categories.slice(0, 8).map((category, index) => <button key={category} className={`reference-category reference-category-${index % 4}`} onClick={() => navigate(`/products?category=${encodeURIComponent(category)}`)}><span className="category-index">{String(index + 1).padStart(2, '0')}</span><span className="category-art"><img src={categoryImagePath(category)} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} /><CircuitIcon index={index} /></span><strong>{category}</strong><small>{products.filter((product) => product.category === category).length} products <ArrowRight size={13} /></small></button>)}</div></div></section>
    {shelf("Trending now", "Popular with builders", trending.length ? trending : stocked, "Shop trending", "sort=featured")}
    <section className="reference-brands section container"><SectionHeading eyebrow="Trusted parts, thoughtfully selected" title="Shop by brand" action="View all brands" onAction={() => navigate('/brands')} /><div className="brand-chip-grid">{(brands.length ? brands.slice(0, 8) : productBrands.map((name) => ({ name, slug: name.toLowerCase().replace(/[^a-z0-9]+/g, '-'), image: '' }))).map((brand, index) => <button key={brand.slug} onClick={() => navigate(`/products?brand=${encodeURIComponent(brand.name)}`)}><span>{String(index + 1).padStart(2, '0')}</span><strong>{brand.name}</strong><ArrowRight size={15} className="arrow-diagonal" /></button>)}</div></section>
...
function ProductsPage({ products, navigate, addToCart }: { products: Product[]; navigate: (path: string) => void; addToCart: (id: string, qty?: number) => void }) {
  ...
  }).sort((a, b) => sortParam === "price-low" ? a.price - b.price : sortParam === "price-high" ? b.price - a.price : sortParam === "new" ? Number(b.newArrival) - Number(a.newArrival) : sortParam === "deals" ? percentOff(b) - percentOff(a) : Number(b.featured) - Number(a.featured)), [products, search, categoryParam, brandParam, sortParam]);
