...
function App() {
  ...
  const renderPage = () => {
    ...
    if (path === "/" || path === "") return <HomePage products={products} site={site} navigate={navigate} addToCart={addToCart} />;
...
function HomePage({ products, site, navigate, addToCart }: { products: Product[]; site: SiteConfig; navigate: (path: string) => void; addToCart: (id: string, qty?: number) => void }) {
  const categories = Array.from(new Set(products.map((product) => product.category))).sort();
