...
function TrackingPage({ site, navigate }: { site: SiteConfig; navigate: (path: string) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!/^ED-\d{8}-\d{4}$/.test(value)) { setOrder(null); setError(bn ? "সঠিক Order ID দিন, যেমন ED-20260924-0001" : "Enter a valid Order ID, for example ED-20260924-0001"); return; }
  ...
  return <div className="page-wrap"><div className="container breadcrumb"><button onClick={() => navigate("/")}>{bn ? "হোম" : "Home"}</button><ChevronRight size={14} /><span>{bn ? "অর্ডার ট্র্যাকিং" : "Order tracking"}</span></div><div className="container tracking-hero"><div className="eyebrow"><span className="eyebrow-line" /> {bn ? "আপনার অর্ডার কোথায়?" : "Your order, clearly tracked"}</div><h1>{bn ? "অর্ডার ট্র্যাক করুন" : "Track your order"}</h1><p>{bn ? "আপনার Order ID লিখে বর্তমান status, total এবং delivery update দেখুন।" : "Enter your Order ID to see the latest status, order summary and delivery update."}</p></div><section className="container order-track-panel"><article className="order-track-card"><h2>{bn ? "Order ID দিয়ে খুঁজুন" : "Find an order"}</h2><p>{bn ? "আপনার order confirmation-এ থাকা ID ব্যবহার করুন।" : "Use the ID from your order confirmation message."}</p><form className="order-track-form" onSubmit={submit}><input value={orderNumber} onChange={(event) => setOrderNumber(event.target.value.toUpperCase())} placeholder="ED-20260924-0001" aria-label="Order ID" /><button className="button button-primary" type="submit" disabled={loading}>{loading ? "Checking…" : (bn ? "Track order" : "Track order")} <ArrowRight size={16} /></button></form>{error && <div className="order-track-result error">{error}</div>}{order && <div className="order-track-result"><div className="order-track-result-head"><strong>{order.orderId}</strong><span className="order-status-pill">{order.status}</span></div><div className="order-track-row"><span>{bn ? "তারিখ" : "Placed"}</span><b>{order.dateTime}</b></div><div className="order-track-row"><span>{bn ? "পণ্য" : "Items"}</span><b>{order.itemSummary} · {order.quantity}</b></div><div className="order-track-row"><span>{bn ? "মোট" : "Total"}</span><b>{formatBDT(order.total)}</b></div><div className="order-track-row"><span>{bn ? "পেমেন্ট" : "Payment"}</span><b>{order.paymentMethod}</b></div></div>}</article><aside className="order-track-card"><h2>{bn ? "Status timeline" : "Order timeline"}</h2><p>{bn ? "আপনার status update Sheet থেকে সরাসরি এখানে দেখা যাবে।" : "Status updates made by our team appear here from the order dashboard."}</p><div className="order-track-timeline">{statuses.map((status, index) => <span key={status} className={statusIndex >= index ? "active" : ""}>{status}</span>)}</div><div className="tracking-help" style={{ margin: "22px 0 0" }}><MessageCircle size={18} /><p>{bn ? "সাহায্য লাগলে WhatsApp-এ যোগাযোগ করুন।" : "Need help with your order? Contact us on WhatsApp."}</p><a href={`https://wa.me/${site.whatsappInternational}`} target="_blank" rel="noreferrer">WhatsApp <ArrowUpRight /></a></div></aside></section><section className="container courier-tracking-grid">{site.shipping.couriers.map((courier) => <article className="courier-tracking-card" key={courier.id}><div className="courier-logo-slot"><img src={courier.logo} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} /><Truck size={30} /></div><div><h2>{courier.name}</h2><p>{bn ? "Courier tracking number পেলে official portal-এ আরও বিস্তারিত tracking করুন।" : "Once you receive a courier tracking number, use the official portal for detailed shipment tracking."}</p></div><a className="button button-primary" href={courier.trackingUrl} target="_blank" rel="noreferrer">{bn ? "Courier tracking" : "Open courier tracking"} <ArrowUpRight /></a></article>)}</section></div>;
}
