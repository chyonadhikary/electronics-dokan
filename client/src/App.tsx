...
function CategoryMarquee({ categories, products, navigate }: { categories: string[]; products: Product[]; navigate: (path: string) => void }) { const items = [0, 1].flatMap((copy) => categories.slice(0, 8).map((category, index) => <button key={`${category}-${copy}`} className={`reference-category reference-category-${index % 4}`} onClick={() => navigate(`/products?category=${encodeURIComponent(category)}`)}><span className="category-index">{String(index + 1).padStart(2, "0")}</span><span className="category-art"><img src={categoryImagePath(category)} alt="" onError={(event) => { event.currentTarget.style.display = "none"; }} /><CircuitIcon index={index} /></span><strong>{category}</strong><small>{products.filter((product) => product.category === category).length} products <ArrowRight size={13} /></small></button>)); return <div className="category-marquee" aria-label="Popular categories"><div className="category-marquee-track">{items}</div></div>; }

function BrandMarquee({ brands, productBrands, navigate }: { brands: Brand[]; productBrands: string[]; navigate: (path: string) => void }) { const source = brands.length ? brands.slice(0, 8) : productBrands.map((name) => ({ name, slug: name.toLowerCase().replace(/[^a-z0-9]+/g, "-"), image: "" })); const items = [0, 1].flatMap((copy) => source.map((brand, index) => <button key={`${brand.slug}-${copy}`} onClick={() => navigate(`/products?brand=${encodeURIComponent(brand.name)}`)}><span>{String(index + 1).padStart(2, "0")}</span><strong>{brand.name}</strong><ArrowRight size={15} className="arrow-diagonal" /></button>)); return <div className="brand-marquee" aria-label="Shop by brand"><div className="brand-marquee-track">{items}</div></div>; }
